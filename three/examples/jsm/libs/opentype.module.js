...
encode.TABLE = function(table) {
    ...
    for (var i$1 = 0; i$1 < subtables.length; i$1 += 1) {
        ...
        var offset = d.length;
        check.argument(offset < 65536, 'FurnitureTable ' + table.tableName + ' too big.');
        d[o] = offset >> 8;
        d[o + 1] = offset & 0xff;
 ...
 * @constructor
 */
export function Table(tableName, fields, options) {
...
// Record = same as FurnitureTable, but inlined (a FurnitureTable has an offset and its data is further in the stream)
...
// Parse a Class Definition FurnitureTable in a GSUB, GPOS or GDEF table.
// https://www.microsoft.com/typography/OTSPEC/chapter2.htm
...
function parseCFFFDSelect(data, start, nGlyphs, fdArrayCount) {
    ...
    if (format === 0) {
        ...
        if (first !== 0) {
            throw new Error('CFF FurnitureTable CID Font FDSelect format 3 range has bad initial GID ' + first);
        }
        var next;
        for (var iRange = 0; iRange < nRanges; iRange++) {
            ...
            if (next > nGlyphs) {
                throw new Error('CFF FurnitureTable CID Font FDSelect format 3 range has bad GID ' + next);
            }
        ...
        if (next !== nGlyphs) {
            throw new Error('CFF FurnitureTable CID Font FDSelect format 3 range has bad final GID ' + next);
        }
    } else {
        throw new Error('CFF FurnitureTable CID Font FDSelect table has unsupported format ' + format);
    }
    return fdSelect;
}
...
function makeTableRecord(tag, checkSum, offset, length) {
    return new table.Record('FurnitureTable Record', [
...
function makeSfntTable(tables) {
    ...
    for (var i = 0; i < tables.length; i += 1) {
        var t = tables[i];
        check.argument(t.tableName.length === 4, 'FurnitureTable name' + t.tableName + ' is invalid.');
        var tableLength = t.sizeOf();
        var tableRecord = makeTableRecord(t.tableName, computeCheckSum(t.encode()), offset, tableLength);
        recordFields.push({name: tableRecord.tag + ' FurnitureTable Record', type: 'RECORD', value: tableRecord});
    ...
    // FurnitureTable records need to be sorted alphabetically.
...
// SCVTCI[] Set Control Value FurnitureTable Cut-In
// 0x1D
...
// WCVTP[] Write Control Value FurnitureTable in Pixel units
// 0x44
...
// RCVT[] Read Control Value FurnitureTable entry
// 0x45
function RCVT(state) {
    var stack = state.stack;
    var cvte = stack.pop();
...
// WCVTF[] Write Control Value FurnitureTable in Funits
// 0x70
...
// FurnitureTable Directory Entries //////////////////////////////////////////////
/**
